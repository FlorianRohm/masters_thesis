...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
Definition of the Laplace transform of centralized $f$, where $u=\begin{pmatrix}u_x \\ u_y\end{pmatrix}$ is the macroscopic speed in the point.
\begin{equation}
  \label{eq:Definition of centralized F}
  \begin{aligned}
    F_c(\Xi_1, \Xi_2) & = \mathcal{L}[f(\cdot - u)](\Xi) = \int_{-\infty}^\infty f(\xi-u) e^{-\Xi \cdot \xi}d\xi \\
    & = \int_{-\infty}^\infty f(\xi) e^{-\Xi \cdot (\xi+u)}d\xi \\
    & = \sum_{i,j}f_{ij} e^{-\Xi_1 (ic+u_x)} e^{-\Xi_2 (jc+u_y)} \\
    & = e^{-\Xi_1 u_x}e^{-\Xi_2 u_y}\sum_{i,j}f_{ij} e^{-\Xi_1 ic} e^{-\Xi_2 jc}\\
    & = e^{-\Xi_1 u_x}e^{-\Xi_2 u_y} \mathcal{L}[f](\Xi).
  \end{aligned}
\end{equation}
...
We seek a representation of the cumulants $c_{\alpha\beta}$ in terms of moments, as the later ones are much more easy to compute, cfg.~\eqref{eq:Definition of moments}. This link can be established with the chain rule for differentiaton.
From~\eqref{eq:alternative representation of moments} and~\eqref{eq:Definition of cumulants} follows
\begin{equation}
  c_{00} = \ln(F(0,0)) = \ln(m_{00}) = \ln(\rho).
\end{equation}
...
\begin{equation*}
  \begin{aligned}
    c_{10} & = {(-c)}^{-1} \left.\partial_1 \ln(F) \right|_{\Xi = 0} \\
    & = {(-c)}^{-1} \left. \frac{1}{F} \partial_1 F \right|_{\Xi = 0} \\
    & = \frac{m_{10}}{m_{00}}
  \end{aligned}
\end{equation*}
The cumulant $c_{01}$ follows from symmetrie and is listed in~\ref{sub:Listing the cumulants from moments}

\begin{align*}
  \phantom{c_{00}}
  &\begin{aligned}
  \nonumber
    \mathllap{c_{11}} & = {(-c)}^{-2} \left.\partial_1 \partial_2 \ln(F) \right|_{\Xi = 0} \\
  ...
  &\begin{aligned}
  \nonumber
  \mathllap{c_{20}} & = {(-c)}^{-2} \left.\partial_1^2 \ln(F) \right|_{\Xi = 0} \\
  ...
  &\begin{aligned}
  \nonumber
  \mathllap{c_{21}} & = {(-c)}^{-3} \left. \partial_2 \partial_1^2 \ln(F) \right|_{\Xi = 0} \\
  ...
  &\begin{aligned}
  \nonumber
  \mathllap{c_{22}} & = {(-c)}^{-4} \left. \partial_2^2 \partial_1^2 \ln(F) \right|_{\Xi = 0} \\
...
\subsection{Listing the cumulants from moments}
\label{sub:Listing the cumulants from moments}
\begin{align}
  c_{00} & = m_{00} \\
  c_{10} & = \frac{m_{10}}{m_{00}} \\
  c_{01} & = \frac{m_{01}}{m_{00}} \\
  c_{11} & = \frac{m_{11}}{m_{00}} - \frac{m_{10}m_{01}}{m_{00}^2} \\
  c_{20} & = \frac{m_{20}}{m_{00}} - \frac{m_{10}^2}{m_{00}^2} \\
  c_{02} & = \frac{m_{02}}{m_{00}} - \frac{m_{01}^2}{m_{00}^2} \\
  c_{21} & = \frac{m_{21}}{m_{00}} - 2\frac{m_{20}m_{01}}{m_{00}^2}
            - 2\frac{m_{10}m_{11}}{m_{00}^2} - 2\frac{m_{10}^2 m_{01}}{m_{00}^3} \\
  c_{12} & = \frac{m_{12}}{m_{00}} - 2\frac{m_{10}m_{02}}{m_{00}^2}
            - 2\frac{m_{11}m_{01}}{m_{00}^2} - 2\frac{m_{10} m_{01}^2}{m_{00}^3} \\
  c_{22} & = \frac{1}{m_{00}} m_{22} \\
    &\quad
    - \frac{1}{m_{00}^2}
    \left(
       2 m_{10}m_{12}  + m_{21}m_{01} + m_{21}m_{01} + 2 m_{11}^2 + m_{20}m_{02}
...
\begin{align}
  ...
  c_{10} & = \hat{m}_{10} \\
  c_{01} & = \hat{m}_{01} \\
  c_{11} & = \hat{m}_{11} - \hat{m}_{10}\hat{m}_{01} \\
  c_{20} & = \hat{m}_{20} - \hat{m}_{10}^2 \\
  c_{02} & = \hat{m}_{02} - \hat{m}_{01}^2 \\
  c_{21} & = \hat{m}_{21} - 2 \hat{m}_{20} \hat{m}_{01}
            - 2 \hat{m}_{10} \hat{m}_{11} - 2 \hat{m}_{10}^2\hat{m}_{01} \\
  c_{12} & = \hat{m}_{12} - 2 \hat{m}_{10} \hat{m}_{02}
