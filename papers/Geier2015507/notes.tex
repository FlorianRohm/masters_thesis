...
\begin{equation}
  \label{eq:first order in epsilon}
  \begin{aligned}
    \frac{\partial}{\partial t} m_{\alpha\beta}^{(0)}
    + m_{\alpha\beta}^{(2)}
    & =  m_{\alpha\beta}^{*(2)}
    - \frac{\partial}{\partial y} m_{\alpha(\beta+1)}^{*(1)}
    + \frac{\partial^2}{\partial y^2} m_{\alpha(\beta+2)}^{*(0)} \\
    &\quad - \frac{\partial}{\partial x} m_{(\alpha+1)\beta}^{*(1)}
    + \frac{\partial^2}{\partial x^2} m_{(\alpha+2)\beta}^{*(0)}
    + \frac{\partial\partial}{\partial x \partial y} m_{(\alpha+1)(\beta+1)}^{*(0)}
  \end{aligned}
\end{equation}
...
Left hand side, either $\tau=1$ and $p=1$ or  $\tau=0$ and $p=3$.
Right hand side:
\begin{center}
  \begin{tabular}{r|c|l}
    m & n & p' \\
