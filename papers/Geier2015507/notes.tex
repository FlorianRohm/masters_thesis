...
If there are no sources and sinks, the density has to be a collision invariant.
\begin{equation}
    \rho  = m_{00} = m_{00}^*
\end{equation}
and thus
...
For the velocities, it's the same:
\begin{equation}
  \begin{aligned}
    u_x & = m_{10} = m_{10}^*
    u_y & = m_{01} = m_{01}^* \\
...
We choose $\epsilon^2$, $\alpha=0$, $\beta=0$ and get
\begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
Also, with $\epsilon^1$, $\alpha=1$, $\beta=0$ we get
\begin{equation}
  \partial_t m_{00}^{(0)} + m_{00}^{(2)} =  m_{00}^{*(2)} - \partial_x m_{10}^{*(1)} - \partial_y m_{01}^{*(1)}
\end{equation}
...
\begin{equation}
  \begin{aligned}
    \frac{\partial}{\partial t} m_{10}^{(1)}
    & =  m_{10}^{*(3)} - m_{10}^{(3)} - \partial_x m_{20}^{*(2)} - \partial_y m_{11}^{*(2)}  \\
    &\quad  + \partial_{xx} m_{30}^{*(1)}/2 + \partial_{xy} m_{21}^{*(1)} + \partial_{yy} m_{12}^{*(1)}/2
  \end{aligned}
\end{equation}
