...
def getCollide(omega):
    def collide(block):
        ...
        rho = getDensity(f,nx,ny)
        u = getVelocity(f,nx,ny)
...
def distributionsFromCentralMoments_min (u, c_00, c_11, c_20, c_02, c_22):
    ...
    f[:,:,0,7] = (c_0_0*(uy**2 - uy) + c_0_1*(2*uy - 1) + c_0_2) * 0.5 # (0, 0)
    f[:,:,0,3] =  c_0_0*(1 - uy**2)  - c_0_1*2*uy       - c_0_2        # (0, 1)
    f[:,:,0,6] = (c_0_0*(uy**2 + uy) + c_0_1*(2*uy + 1) + c_0_2) * 0.5 # (0, 2)
    f[:,:,0,4] = (c_1_0*(uy**2 - uy) + c_1_1*(2*uy - 1) + c_1_2) * 0.5 # (1, 0)
    f[:,:,0,0] =  c_1_0*(1 - uy**2)  - c_1_1*2*uy       - c_1_2        # (1, 1)
    f[:,:,0,2] = (c_1_0*(uy**2 + uy) + c_1_1*(2*uy + 1) + c_1_2) * 0.5 # (1, 2)
    f[:,:,0,8] = (c_2_0*(uy**2 - uy) + c_2_1*(2*uy - 1) + c_2_2) * 0.5 # (2, 0)
    f[:,:,0,1] =  c_2_0*(1 - uy**2)  - c_2_1*2*uy       - c_2_2        # (2, 1)
    f[:,:,0,5] = (c_2_0*(uy**2 + uy) + c_2_1*(2*uy + 1) + c_2_2) * 0.5 # (2, 2)

    return f

def getVelocity(f,nx,ny):
    ux = f[:,:,0,1] - f[:,:,0,3] + f[:,:,0,5] - f[:,:,0,7] + f[:,:,0,8] - f[:,:,0,6]
    uy = f[:,:,0,2] - f[:,:,0,4] + f[:,:,0,6] - f[:,:,0,8] + f[:,:,0,5] - f[:,:,0,7]
    return array((ux,uy))
