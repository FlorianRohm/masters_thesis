...
\begin{align}
  m_{\alpha\beta} &= \sum_{i,j \in \{-1,0,1\}} c^{\alpha + \beta} i^\alpha j^\beta f_{ij}
  ...
  u_x  & = \frac{1}{\rho} \sum_{i,j \in \{-1,0,1\}} ci f_{ij} = \frac{m_{10}}{m_{00}}
  \label{eq: x velocity definition}\\
  u_y  & = \frac{1}{\rho} \sum_{i,j \in \{-1,0,1\}} cj f_{ij} = \frac{m_{01}}{m_{00}}.
...
\begin{equation}
  m_{30} = \sum_{i,j \in \{-1,0,1\}} i^3 f_{ij} = \sum_{i,j \in \{-1,0,1\}} i i^2 f_{ij} = \sum_{i,j \in \{-1,0,1\}} i f_{ij} = m_{10},
\end{equation}
or in general
\begin{equation}
  \label{eq:aliasing of moments 1}
  m_{\alpha\beta}
  = \sum_{i,j \in \{-1,0,1\}} i^\alpha j^\beta f_{ij}
  = \sum_{i,j \in \{-1,0,1\}} i^{(\alpha+2k)} j^\beta f_{ij}
  = m_{(\alpha+2k)\beta} \quad \forall k,\beta, \forall \alpha \neq 0
\end{equation}
and
\begin{equation}
  ...
  m_{\alpha\beta}
  = \sum_{i,j \in \{-1,0,1\}} i^\alpha j^\beta f_{ij}
  = \sum_{i,j \in \{-1,0,1\}} i^\alpha j^{(\beta+2l)} f_{ij}
  = m_{\alpha(\beta+2l)} \quad \forall l, \alpha, \forall \beta \neq 0.
\end{equation}
