...
\begin{equation}
  \label{eq: lattice boltzmann equation}
    ...
    = f_{ij}^*(x, y, t)
\end{equation}
for all $i,j\in \{-1, 0, 1\}$, where $f_{ij}^*$ are the post-collision distributions defined as
\begin{equation}
  \label{eq: define post collision}
  f_{ij}^*(x, y, t) \defined \tilde{\Omega}(f_{ij}(x,y,t)) + f_{ij}(x,y,t).
\end{equation}
The $f_{ij}$ are accordingly labeled pre-collision distributions.
